...
The Java UI distinguishes two LDAP roles: `administrator` and `user`. The administrator sees all Spark jobs and Spark UI; the user sees only applications whose `sparkUser` matches their login and cannot open a shared or another user's `/spark-ui/**` by direct URL. SQL, HDFS, Ozone, and HBase run under the user's Kerberos identity, while the corresponding cluster service retains the final decision on data access.
...
```bash
cd kudos
./dev/scripts/test-sql-engines.sh
```
...
```bash
cd kudos
columns (horizontal scrolling and a limit on visible columns). The scale is
parameterized; the defaults are a representative matrix that completes in a few
```
...
`admin` belongs to the `kudos-administrators` LDAP group and receives the `administrator` role; `analyst` receives the `user` role. All these passwords are stored in plain text in `compose.yaml` and the systemd bootstrap unit and are permitted only for the isolated local test environment.
...
dev/docker/app/                      Java multi-stage image
dev/docker/freeipa/                  FreeIPA image and identity bootstrap
dev/docker/kyuubi/                   Kerberos Kyuubi/Spark configuration
