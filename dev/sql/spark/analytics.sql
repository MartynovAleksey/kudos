...
-- Spark SQL (Kyuubi) analytics over the shared Iceberg lakehouse.
-- Dataset: dev/scripts/lakehouse-loadgen.sh (customers -> orders -> order_items).
-- The `iceberg` catalog is pre-wired in kyuubi-defaults.conf, so tables are
-- addressed as iceberg.<namespace>.<table>. Same logic as the Flink/Trino/
-- StarRocks scripts — results must match for one dataset snapshot.
