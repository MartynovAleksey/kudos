...
class EngineVisibilityControllerTests {

  @Autowired MockMvc mockMvc;
  @Autowired EngineVisibilityStore store;
  ...
  void anySignedInUserCanReadTheVisibility() throws Exception {
    mockMvc
        .perform(get("/ui-api/engine-visibility"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.trino").value(true));
  }

  @Test
  @WithMockUser(username = "admin", authorities = "ROLE_ADMINISTRATOR")
  void anAdministratorCanHideAnEngine() throws Exception {
    mockMvc
        .perform(
            put("/ui-api/engine-visibility")
                .with(csrf())
                .contentType(MediaType.APPLICATION_JSON)
                .content("{\"trino\":false}"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.trino").value(false))
        .andExpect(jsonPath("$.kyuubi").value(true));
  }
